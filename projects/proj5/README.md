...
- **CountryDefinitionList**描述俄克拉何马州内的所有区县。构造函数负责从一个CSV文件读取区县数据，创建区县列表，和处理经纬度坐标和屏幕坐标间的转换（转换涉及一个按比例缩放和一个转换）。请完成这个构造函数。完成*getBounds()*，*translate()*，*draw()*，*addStations()*，*colorize()*和*findCounty()*方法。
